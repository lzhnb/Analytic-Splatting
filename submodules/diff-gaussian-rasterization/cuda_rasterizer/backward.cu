...
renderCUDA(
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
			...
			const float4 cov_o = collected_cov_opacity[j];
			...
			const const float2 v1 = {cov_o.y, lambda1 - cov_o.x}; // equal to {lambda1 - cov_o.z, cov_o.y}
			...
			const float2 v2 = {lambda2 - cov_o.z, cov_o.y}; // equal to {cov_o.y, lambda2 - cov_o.x}
			...
			const float alpha = min(0.99f, cov_o.w * G);
			...
			/* Here is the backward calculation in supplement */
			// Gradient source
			const float dL_dG = cov_o.w * dL_dalpha;
			...
			const float dL_dmean2Dx = dL_dG * dG_ddelx * ddelx_dx;
			const float dL_dmean2Dy = dL_dG * dG_ddely * ddely_dy;
			atomicAdd(&dL_dmean2D[global_id].x, dL_dmean2Dx);
			atomicAdd(&dL_dmean2D[global_id].y, dL_dmean2Dy);
			atomicAdd(&dL_dmean2D[global_id].z, sqrtf(dL_dmean2Dx * dL_dmean2Dx + dL_dmean2Dy * dL_dmean2Dy));
