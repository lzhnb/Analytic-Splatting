...
class GaussianModel:
    ...
    def construct_list_of_attributes(self, exclude_filter: bool = False) -> List[str]:
        l = ["x", "y", "z", "nx", "ny", "nz"]
    ...
    def save_fused_ply(self, path):
        ...
        scale = (
            self.scaling_inverse_activation(self.get_scaling_with_3D_filter).detach().cpu().numpy()
        )
        ...
        dtype_full = [
            (attribute, "f4")
            for attribute in self.construct_list_of_attributes(exclude_filter=True)
        ]
    ...
    def densify_and_split(
        ...
        grad_abs_threshold: torch.Tensor,
        scene_extent: float,
        N: int = 2,
        dense: bool = False,
    ) -> None:
    ...
    def densify_and_prune(
        ...
        self.densify_and_clone(grads, max_grad, grads_abs, Q, extent, dense=dense)
        self.densify_and_split(grads, max_grad, grads_abs, Q, extent, dense=dense)
