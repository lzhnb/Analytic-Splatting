...
from scene import Camera, GaussianModel, Scene
...
def training(
    ...
    ref_origin_camera: Camera = scene.train_cameras[1.0][0]
    origin_width = ref_origin_camera.image_width
    for index, camera in enumerate(trainCameras):
        if camera.image_width == origin_width:
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if (random() < 0.3) and (idx not in highresolution_index) and dataset.sample_more_highres:
        #     viewpoint_cam = trainCameras[highresolution_index[randint(0, len(highresolution_index)-1)]]
        if random() < 0.3 and dataset.sample_more_highres:
            viewpoint_cam = trainCameras[
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                # # NOTE: expand low-resolution's radii (3D Gaussian is a small Gaussian in )
                # scale = origin_width / viewpoint_cam.image_width
                # radii = radii * scale
...
def training_report(
    ...
    scaling_grad: torch.Tensor,
    opacity_grad: torch.Tensor,
    elapsed: float,
    testing_iterations: List[int],
    scene: Scene,
    ...
    if tb_writer:
        ...
        tb_writer.add_scalar("train_gard/position", xyz_grad.abs().mean(1).sum().item(), iteration)
        tb_writer.add_scalar(
            "train_gard/rotation", rotation_grad.abs().mean(1).sum().item(), iteration
        )
        tb_writer.add_scalar(
            "train_gard/scaling", scaling_grad.abs().mean(1).sum().item(), iteration
        )
        tb_writer.add_scalar("train_gard/opacity", opacity_grad.abs().sum().item(), iteration)
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    image = torch.clamp(
                        render(viewpoint, scene.gaussians, *renderArgs, filter3d=filter3d)[
                            "render"
                        ],
                        0.0,
                        1.0,
                    ...
                    if tb_writer and (idx < 5):
                        tb_writer.add_images(
                            f"{name}_view_{viewpoint.image_name}/render",
                        ...
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(
                                f"{name}_view_{viewpoint.image_name}/ground_truth",
                ...
                print(
                    ...
                    f"L1 {l1_test:.6f} PSNR {psnr_test:.6f}"
                )
                if tb_writer:
